	...
	public:
		...
		void DrawMesh(float deltaTime, entt::registry &registry)
		{
			...
			for (RenderEnttRapper rer : sortingEntities)
			{
				...
				{
					AssetManager::Get<ModelAsset>(modelId)->UpdateBones(
						bones,
						AssetManager::Get<ModelAsset>(modelId)->animationClips[0],
						std::fmod(m_time, AssetManager::Get<ModelAsset>(modelId)->animationClips[0].duration));
					std::cout << std::fmod(m_time, AssetManager::Get<ModelAsset>(modelId)->animationClips[0].duration) << std::endl;
					
					shadow_mapping_shader->SetBool("USEBONES", true);

					std::vector<glm::mat4> b = std::vector<glm::mat4>(32);

					for (int i = 0; i < b.size(); ++i)
						if (i < bones.size())
							shadow_mapping_shader->SetMat4("boneTransforms[" + std::to_string(i) + "]", bones[i].finalMatrix);
						else
							shadow_mapping_shader->SetMat4("boneTransforms[" + std::to_string(i) + "]", glm::mat4(1));
				}
				else
				{
					shadow_mapping_shader->SetBool("USEBONES", false);
				}
