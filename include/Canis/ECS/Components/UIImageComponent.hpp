...
namespace Canis
{
    ...
    {
        _sprite.uv.x = (_flipX) ? (((_row+1) * _spriteWidth) + _offsetX)/(float)_sprite.textureHandle.texture.width : (_row == 0) ? 0.0f : ((_row * _spriteWidth) + _offsetX)/(float)_sprite.textureHandle.texture.width;
        _sprite.uv.y = (_flipY) ? (((_column+1) * _spriteHeight) + _offsetY)/(float)_sprite.textureHandle.texture.height : (_column == 0) ? 0.0f : ((_column * _spriteHeight) + _offsetY)/(float)_sprite.textureHandle.texture.height;
