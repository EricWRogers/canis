...
namespace Canis
{
    ...
    {
        _sprite.uv.x = (_flipX) ? (((_indexX+1) * _spriteWidth) + _offsetX)/(float)_sprite.texture.width : (_indexX == 0) ? 0.0f : ((_indexX * _spriteWidth) + _offsetX)/(float)_sprite.texture.width;
        _sprite.uv.y = (_flipY) ? (((_indexY+1) * _spriteHeight) + _offsetY)/(float)_sprite.texture.height : (_indexY == 0) ? 0.0f : ((_indexY * _spriteHeight) + _offsetY)/(float)_sprite.texture.height;
