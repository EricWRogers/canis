    ...
    namespace
    {
            ...
            std::vector<PropertyUniformInfo> &_outPropertyUniforms)
        {
            ...
            shader
                ...
                << "out vec4 fragmentInstanceColor;\n"
                << "void main()\n"
                << "{\n"
                << "    mat4 sgModel = useInstanceMatrix ? instanceModel : M;\n"
                << "    fragmentInstanceColor = useInstanceColor ? instanceColor : vec4(1.0);\n"
                << "    vec4 sgWorldPos = sgModel * vec4(vertexPosition, 1.0);\n"
                << "    mat3 sgNormalMatrix = mat3(transpose(inverse(sgModel)));\n"
...
vec3 sgToonLighting(vec3 normal,float bands) {
    return sgToonLighting(normal,bands,0.0);
                ...
                << "    sgSurfaceColor *= albedoValue * fragmentInstanceColor;\n"
